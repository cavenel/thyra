...
class TestOptimizeZarrChunks:
    ...
    def test_optimize_zarr_chunks_with_output_path(self, mock_da, mock_zarr, temp_dir):
        ...
        # Run optimization
        result = optimize_zarr_chunks(str(zarr_path), array_path, output_path=str(output_path))
    ...
    @patch('msiconvert.utils.data_processors.zarr')
    @patch('msiconvert.utils.data_processors.da')
    def test_optimize_zarr_chunks_with_custom_chunks(self, mock_da, mock_zarr, temp_dir):
    ...
    def test_optimize_zarr_chunks_with_compressor(self, mock_da, mock_zarr, temp_dir):
        ...
        # Run optimization
        result = optimize_zarr_chunks(str(zarr_path), array_path, compressor=mock_compressor)
    ...
    def test_optimize_zarr_chunks_with_rename(self, mock_da, mock_zarr, temp_dir):
        ...
        mock_store.__setitem__.assert_any_call(f"{array_path}_backup", mock_store.__getitem__.return_value)
        del mock_store[array_path]  # Should have been called
        mock_store.__setitem__.assert_any_call(array_path, mock_store.__getitem__.return_value)
