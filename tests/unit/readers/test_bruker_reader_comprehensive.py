...
from msiconvert.readers.bruker.core.exceptions import BrukerReaderError, SDKError, DataError
    ...
    def test_spectrum_reading(self) -> Dict[str, Any]:
        ...
        with BrukerReader(self.test_data_path) as reader:
            ...
            for coords, mzs, intensities in reader.iter_spectra():
                ...
                assert len(mzs) == len(intensities), "m/z and intensity arrays should have same length"
                assert np.all(mzs >= 0), "m/z values should be non-negative"
                assert np.all(intensities >= 0), "Intensity values should be non-negative"
    ...
    def test_batch_processing(self) -> Dict[str, Any]:
        ...
        with BrukerReader(self.test_data_path) as reader:
            ...
            for batch_size in batch_sizes:
                ...
                for coords, mzs, intensities in reader.iter_spectra(batch_size=batch_size):
                    spectrum_count += 1
