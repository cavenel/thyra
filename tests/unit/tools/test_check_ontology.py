...
class TestCheckOntology:
    ...
    @patch(
        "msiconvert.metadata.validator.ImzMLOntologyValidator", autospec=True
    )
    @patch("sys.argv")
    def test_main_file_check(
        ...
        dummy_imzml.write_text(
            """<mzML><cvList><cv id="MS" fullName="MS" version="1.0" uri="http://example.com/ms.obo"/></cvList><run><spectrumList count="0"/></run></mzML>"""
        )
        ...
        try:
            ...
            mock_validator_instance.validate_file.assert_called_once_with(
                dummy_imzml
            )
