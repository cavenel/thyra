...
class ImzMLOntologyValidator:
    ...
    def validate_file(self, imzml_path: Path) -> Dict[str, Any]:
        ...
        for cv_param in cv_params:
            ...
            if accession: # Ensure we don't count empty accessions
                results['term_counts'][accession] = results['term_counts'].get(accession, 0) + 1
            # ---------------------------------------------------
            ...
            if term:
                ...
                context = cv_param.get('..', {}).tag if hasattr(cv_param, 'get') else 'unknown'
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            f"Known terms: {results['known_terms']} ({results['known_terms']/results['total_terms']*100:.1f}%)" if results['total_terms'] > 0 else "Known terms: 0 (0.0%)",
            f"Unknown terms: {results['unknown_terms']} ({results['unknown_terms']/results['total_terms']*100:.1f}%)" if results['total_terms'] > 0 else "Unknown terms: 0 (0.0%)",
        ]
        ...
        if results['term_counts']:
            ...
            sorted_terms = sorted(results['term_counts'].items(), key=lambda item: item[1], reverse=True)
            for accession, count in sorted_terms[:15]: # Display top 15
                term_details = ONTOLOGY.get_term(accession)
                
                # --- THIS IS THE CORRECTED LINE ---
                term_name = term_details[1] if term_details and len(term_details) > 1 else "Unknown Term"
                # ------------------------------------
