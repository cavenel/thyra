...
class OntologyCache:
    ...
    def __init__(self):
        ...
        self.name_to_accession = {
            name.lower(): acc 
            for acc, (name, _) in self.terms.items()
        }
        
        logger.info(f"Loaded {len(self.terms)} ontology terms")
        logger.info(f"  - MS terms: {len([k for k in self.terms if k.startswith('MS:')])}")
        logger.info(f"  - IMS terms: {len([k for k in self.terms if k.startswith('IMS:')])}")
        logger.info(f"  - UO terms: {len([k for k in self.terms if k.startswith('UO:')])}")
