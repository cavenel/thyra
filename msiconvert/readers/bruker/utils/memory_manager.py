...
class MemoryManager:
    ...
    def check_memory_limit(self) -> bool:
        ...
        if current_gb > self.memory_limit_gb:
            logger.warning(f"Memory limit exceeded: {current_gb:.2f}GB > {self.memory_limit_gb}GB")
            return False
    ...
    def allocate_spectrum_buffers(self, estimated_peaks: int) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def return_spectrum_buffers(self, mz_buffer: np.ndarray, 
                               intensity_buffer: np.ndarray) -> None:
        """
    ...
    def estimate_memory_requirements(self, n_spectra: int, 
                                   avg_peaks_per_spectrum: int,
                                   n_unique_masses: int) -> Dict[str, float]:
        """
        ...
        raw_data_mb = (n_spectra * avg_peaks_per_spectrum * 
                      (8 + 4)) / 1024 / 1024  # 8 bytes for m/z, 4 for intensity
        ...
        total_mb = raw_data_mb + mass_axis_mb + sparse_matrix_mb + coordinates_mb + buffer_overhead_mb
    ...
    def suggest_batch_size(self, total_spectra: int, 
                          avg_peaks_per_spectrum: int,
                          target_memory_mb: float = 512) -> int:
        """
        ...
        memory_per_spectrum_mb = (avg_peaks_per_spectrum * 12) / 1024 / 1024  # 12 bytes per peak
        ...
        logger.info(f"Suggested batch size: {batch_size} (target: {target_memory_mb}MB)")
        return batch_size
