...
import numpy as np
from typing import List, Optional, Iterator, Set, Dict, Any
import logging
...
class MassAxisBuilder:
    ...
    def build_from_spectra_iterator(self, spectra_iterator: Iterator,
                                  total_spectra: Optional[int] = None) -> np.ndarray:
        """
    ...
    def _determine_optimal_strategy(self, total_spectra: Optional[int]) -> str:
        ...
        estimated_memory_mb = estimated_total_peaks * 8 / 1024 / 1024  # 8 bytes per m/z value
    ...
    def _build_fast(self, spectra_iterator: Iterator, 
                   total_spectra: Optional[int]) -> np.ndarray:
        """
        ...
        # Setup progress bar
        pbar = tqdm(total=total_spectra, desc="Building mass axis (fast)", 
                   unit="spectrum", disable=not total_spectra)
        ...
        try:
            for coords, mzs, intensities in spectra_iterator:
                ...
                if self.progress_callback:
                    self.progress_callback(self._stats['total_spectra_processed'], total_spectra)
        ...
        logger.info(f"Found {len(unique_mzs)} unique m/z values from {len(all_mzs)} spectra")
        
        return unique_mzs
    
    def _build_memory_efficient(self, spectra_iterator: Iterator,
                               total_spectra: Optional[int]) -> np.ndarray:
        """
        ...
        pbar = tqdm(total=total_spectra, desc="Building mass axis (memory efficient)",
                   unit="spectrum", disable=not total_spectra)
    ...
    def _build_streaming(self, spectra_iterator: Iterator,
                        total_spectra: Optional[int]) -> np.ndarray:
        """
        ...
        pbar = tqdm(total=total_spectra, desc="Building mass axis (streaming)",
                   unit="spectrum", disable=not total_spectra)
        ...
        try:
            for coords, mzs, intensities in spectra_iterator:
                ...
                if batch_count % batch_size == 0:
                    logger.debug(f"Processed {self._stats['total_spectra_processed']} spectra, "
    ...
    def build_with_tolerance(self, spectra_iterator: Iterator,
        ...
        logger.info(f"Consolidated from {len(unique_mzs)} to {len(consolidated_mzs)} m/z values")
        return consolidated_mzs
    
    def _consolidate_with_tolerance(self, mzs: np.ndarray, 
                                  tolerance_ppm: float) -> np.ndarray:
        """
