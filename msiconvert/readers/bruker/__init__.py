...
from .core.exceptions import BrukerReaderError, SDKError, DataError, FileFormatError
...
class BrukerReader(BaseMSIReader):
    ...
    def __init__(self, data_path: Path, 
        ...
        logger.info(f"Initialized Ultimate BrukerReader for {self.file_type.upper()} data at {data_path}")
    ...
    def _preload_metadata(self):
        """Preload metadata for compatibility with tests."""
    ...
    def _setup_components(self, cache_coordinates: bool, 
    ...
    def _iter_spectra_raw(self) -> Generator[
        Tuple[Tuple[int, int, int], np.ndarray, np.ndarray], None, None
    ]:
    ...
    def _iter_spectra_batched(self, batch_size: int) -> Generator[
        Tuple[Tuple[int, int, int], np.ndarray, np.ndarray], None, None
    ]:
        ...
        for batch_results in self.batch_processor.process_spectrum_batches(
            iter([frame_ids[i:i+batch_size] for i in range(0, len(frame_ids), batch_size)]),
