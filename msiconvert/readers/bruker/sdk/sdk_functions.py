...
from ctypes import (
    c_double, c_float, c_int32, c_int64, c_uint32, c_uint64, 
...
class SDKFunctions:
    ...
    def _setup_tsf_functions(self, dll) -> None:
        ...
        dll.tsf_read_line_spectrum_v2.argtypes = [
            c_uint64, c_int64, POINTER(c_double), POINTER(c_float), c_int32
        ]
    ...
    def _setup_tdf_functions(self, dll) -> None:
        ...
        dll.tims_read_scans_v2.argtypes = [
            c_uint64, c_int64, c_uint32, c_uint32, POINTER(c_uint32), c_uint32
        ]
    ...
    def open_file(self, file_path: str, use_recalibrated: bool = False) -> int:
        ...
        if self.file_type == 'tsf':
            handle = dll.tsf_open(file_path.encode('utf-8'), 1 if use_recalibrated else 0)
        else:  # tdf
            handle = dll.tims_open(file_path.encode('utf-8'), 1 if use_recalibrated else 0)
        
        if handle == 0:
            error_msg = self._get_last_error()
            raise SDKError(f"Failed to open {self.file_type.upper()} file: {error_msg}")
        
        logger.debug(f"Opened {self.file_type.upper()} file: {file_path} (handle: {handle})")
        return handle
    ...
    def read_spectrum(self, handle: int, frame_id: int, 
                     buffer_size: int = 1024) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _read_tsf_spectrum(self, handle: int, frame_id: int, 
                          buffer_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _read_tdf_spectrum(self, handle: int, frame_id: int, 
                          buffer_size: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        scan_data = self._read_scans(handle, frame_id - 1, 0, 100)  # Read first 100 scans
    ...
    def _read_scans(self, handle: int, frame_id: int, scan_begin: int, 
                   scan_end: int) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    def _convert_indices_to_mz(self, handle: int, frame_id: int, 
