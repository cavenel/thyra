...
class BaseSpatialDataConverter(BaseMSIConverter, ABC):
    ...
    def _nearest_neighbor_resample(
        ...
        """Resample using nearest neighbor interpolation without tolerance threshold."""
        if mzs.size == 0:
            return np.zeros(len(self._common_mass_axis))

        # Initialize resampled array
        resampled = np.zeros(len(self._common_mass_axis))

        # For each original peak, find the nearest bin in resampled axis
        for i, original_mz in enumerate(mzs):
            # Find closest bin in the resampled axis
            closest_bin_idx = np.argmin(np.abs(self._common_mass_axis - original_mz))
            # Accumulate intensity (in case multiple peaks map to same bin)
            resampled[closest_bin_idx] += intensities[i]
    ...
    def _tic_preserving_resample(
        ...
        """Resample using TIC-preserving interpolation (linear interpolation)."""
        if mzs.size == 0:
            return np.zeros(len(self._common_mass_axis))

        # Use numpy interp for linear interpolation
        # Ensure mzs are sorted for interpolation
        sort_indices = np.argsort(mzs)
        mzs_sorted = mzs[sort_indices]
        intensities_sorted = intensities[sort_indices]

        # Interpolate onto the common mass axis
        resampled = np.interp(
